...
import { Layers, ArrowRight, Grid, Sparkles, ChevronRight, Home, PackageX } from 'lucide-react';
  ...
  useEffect(() => {
    async function loadCategories() {
      try {
        ...
        setError(err?.message || 'Failed to fetch hardware categories.');
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-slate-900 text-white rounded-[32px] p-8 sm:p-12 mb-10 relative overflow-hidden border border-slate-800 shadow-xl">
          <div className="relative z-10 max-w-2xl space-y-4">
            ...
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight leading-tight">
              Hardware & Audio Categories
            </h1>
            <p className="text-slate-300 text-sm leading-relaxed font-normal">
              Browse our complete catalog organized by specialized precision departments. From spatial audio headsets to custom mechanical keyboards and high-output power hubs.
            </p>
          </div>
        ...
        {!loading && !error && categories.length === 0 && (
          <div className="bg-white rounded-3xl p-12 border border-slate-200 text-center max-w-lg mx-auto my-12 shadow-sm space-y-4">
            ...
            <p className="text-xs text-slate-500">Categories are currently being updated by inventory controls.</p>
            <Link 
              href="/products" 
              ...
              >
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed mt-2 line-clamp-2 font-medium">
                      {cat.description || 'Explore high performance electronics and workstation hardware.'}
                    </p>
                  </div>
                </div>
